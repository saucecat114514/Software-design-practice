...
- **业务流程**：
  ...
  5. 画像聚合内容必须包含：家长身份信息、关联的所有孩子档案、历史咨询/试听/报价/跟进记录、最新意向等级、归属顾问、转介绍来源。
...
- **追溯**：REQ-CRM-001, REQ-CRM-002；用例：查询客户画像与历史。
...
- **业务流程**：
  ...
  3. 若命中已有客户，系统不阻断录入，但强制弹窗提示：
     - a. 该手机号已存在的归属顾问姓名；
     - b. 最近一次跟进记录的摘要；
     - c. 顾问可选择仍作为“潜在冲突”新建，或放弃转入原客户详情页。
  4. 若未命中，正常完成建档。
...
- **异常处理**：
  - 名额查询接口超时或失败：界面提示“名额信息加载失败，请刷新重试”，并禁用锁定按钮。
- **验收标准**：
  - 展开班级详情到显示剩余名额的时间≤2秒。
...
- **业务流程**：
  ...
  3. 顾问需手动标注“家长意向优先级”（非此时段不可/优先此时段/任意时段均可），系统根据报名来源自动判断队列类型：
     - a. 付费试听报名学员进入“高级队列”；
     - b. 普通咨询学员进入“普通队列”。
  4. 提交后，学员加入候补队列，按综合优先级排序。
  5. 当该时段名额因取消、释放等原因回池时，系统立即（1分钟内）按排序从候补队列中取出首个匹配的学员，并向其归属顾问发送补位通知。
- **业务规则**：
  - BR-TRIAL-008: 试听候补队列综合排序权重（由高到低）：队列类型（高级队列 > 普通队列） > 家长意向优先级（非此时段不可 > 优先此时段 > 任意时段均可） > 进入时间。
...
- **业务流程**：
  ...
  4. 系统根据预置三级路由规则，判定审批流：
     - a. **规则A（小额高效）**：订单金额 ≤ 5000元 且 折扣 ≥ 约定折扣， → 路由至**校区主管**。30分钟内主管未审批，系统自动通过并标记为“异常通过”，同步抄送校长与财务。
     - b. **规则B（大额风控）**：订单金额 > 约定金额 或 折扣 < 约定折扣 或 课时数 > 约定课时， → 路由至**校长**。工作日4小时内审批，非工作日8小时内审批，绝不自动通过。
     - c. **规则C（代理人）**：规则B的校长审批单，若5分钟内未处理，系统提醒代理人；30分钟未处理，升级至值班校长；代理人15分钟未处理，订单自动拒绝。
  5. 审批结果以强提醒方式（声音+震动，由通知中心配置）推送给顾问。
- **业务规则**：
  - BR-ORDER-005: 审批三级路由阈值：金额 ≤ 5000元 且 折扣 ≥ 9折 → 校区主管审批。金额 > 10000元 或 折扣 < 85折 或 课时 > 150课时 → 校长审批。
  - BR-ORDER-006: 审批时限：主管审批超30分钟自动通过；校长审批超时（工作日4h/非工作日8h）自动拒绝或触发升级，不自动通过。
...
- **业务规则**：
  - BR-ORDER-011: 系统严禁在顾问/教务端提供任何线下收款（现金/POS机/个人转账）的操作入口。
...
- **业务流程**：
  ...
  2. 系统自动计算**占用时段**＝课程开始前缓冲时间 + 课程时段 + 课程结束后缓冲时间（缓冲时间默认 5 分钟，可按教室对配置，参见 FR-SCH-006）。
...
- **业务流程**：
  ...
  3. 若匹配成功且学员处于可签到状态，系统自动记录签到，签到时间取服务器时间，标记状态为“出勤”，并扣减对应课时（依据迟到等规则可能不扣或部分扣）。
  4. 若学员迟到（签到时间晚于课程开始时间），系统自动计算迟到时长：
     - 迟到 ≤ 15 分钟：状态标记为“出勤（迟到）”，不扣课时，记录迟到时长。
     - 迟到 > 15 分钟：状态标记为“迟到”，仍算到课，但按课程规则正常扣课时（整次），并记录迟到时长。
  5. 若学员提前离场需签退：家长可再次扫码签退，系统记录离场时间。若离场时间早于课程结束时间且未请假，则状态标记为早退”，并按照整课时扣除课时（不减半），记录离场时间。
  6. 任何签到签退操作均实时更新学员剩余课时及教师课时费预统计（依据实际考勤结果）。
- **业务规则**：
  - 迟到时长 = 签到时间 – 课程开始时间；若迟到 ≤ 15 分钟，状态为“出勤（迟到）”，不扣课时；迟到 > 15 分钟，状态为“迟到”，扣整次课时。
  - 早退判定：签退时间早于排课结束时间且未请假，状态标记为“早退”，课时整次扣除。
  - 课时扣减以“是否到课”为准，迟到/早退不影响扣课数量（除迟到≤15分钟外），仅记录时间（CCB ISSUE-010 状态机）。
...
- **验收标准**：
  1. 模拟迟到 10 分钟签到，状态为“出勤（迟到）”，剩余课时不变。
  2. 迟到 20 分钟签到，状态为“迟到”，课时扣减 1 次。
...
- **业务流程**：
  ...
  2. 系统判断当前时间与课程开始时间的关系：
     ...
     - 若距离课程开始 ≤ 2 小时且 > 0：仅允许提交“临时病假”，需上传合规诊断证明（3 个工作日内有效）；若未上传，申请被拒绝。
  3. 家长填写请假原因并上传证明（如需），提交后系统生成请假申请单。
  4. 对于普通请假（提前 > 2 小时），系统自动审批通过，状态更新为“已请假”，不扣课时，同时自动为该学员生成等额补课额度（参见 FR-MAKEUP-001）。
  5. 对于临时病假（提前 ≤ 2 小时），需要教务老师后台审批。教务老师核查诊断证明，通过后状态更新为“临时病假”，按配置比例扣除课时（默认 50%，后台可配）；若不通过，则状态为“缺勤”，扣除整次课时。
  6. 任何请假审批生效后，系统自动通知家长，并触发补课额度计算（如适用）。
- **业务规则**：
  - 普通请假截止时刻 = 课程开始时间 - 2 小时。
  - 临时病假扣费比例默认 50%，可后台配置（0-100%）。
  - 请假审批通过后，补课额度立即生成，有效期自请假审批通过日起算 3 个月（CCB ISSUE-012）。
  - 临时病假诊断证明必须为合规机构出具，且日期在课程当日前 3 个工作日至课程日之间；否则审批拒绝。
...
- **验收标准**：
  ...
  2. 提前 1 小时提交临时病假，上传有效诊断证明，审批通过后课时扣除 50%。
...
- **业务流程**：
  ...
  4. 保存成功后：
     ...
     - 追踪每条通知的已读状态，对次日仍未读的家长生成待办清单供教务人工跟进。
...
- **业务流程**：
  ...
  4. **教师课时明细报表**：支持按教师、班级、学员三种维度查询和导出。
     - 按教师：显示教师姓名、学员姓名、班级、课程、上课日期与时段、课时费单价、代课标记（是则显示原定教师）、补课原因（教师/学员）、备注。底部汇总总课时与总课时费。
...
- **业务流程**：
  ...
  4. 家长确认后，系统提交退费审批：
     - 退费金额 ≤ 5000 元且退费比例不低于 9 折（即退费金额/剩余价值 ≥ 0.9）且不涉及超 150 课时的大单：进入**校区主管审批**，30 分钟超时自动通过，并标记“异常通过”抄送校长/财务。
     - 退费金额 > 10000 元或退费折扣 < 85 折或课时 > 150 或涉及捆绑销售等复杂场景：进入**校长强制审批**，工作日 4h、非工作日 8h 内处理，绝不自动通过；若审批人 5 分钟未响应转值班校长，30 分钟未处理转代理人，代理人 15 分钟仍未处理则自动拒绝。
     - 中间档次按就高不就低原则。
...
- **业务规则**：
  - 退费单价锁定不变，多次退费保持一致。
  - 赠课一律不折现，只作废。
  - 审批路由严格按财务规则： ≤5000且≥9折且≤150课时走主管审批； >10000或<85折或>150课时走校长审批；中间地带归属校长审批（最严），具体阈值见CCB ISSUE-008。
...
- **业务流程**：
  1. 用户可全局查看所有有权限的校区数据，或切换至特定单一校区。
...
- **业务流程**：
  ...
  2. 家长填写转账信息：收款人姓名、转账金额、转账时间、付款人账户名、上传转账凭证截图。
  3. 提交后，系统根据“收款人姓名+金额”规则，在学员的未支付订单中自动尝试匹配。
...
- **输入/输出**：
  - 输入：学籍号、收款人姓名、金额、时间、凭证。
  - 输出：匹配成功的提示，或“您的付款信息已提交，我们将尽快核实”的存疑提示。
...
- **业务规则**：
  ...
  - **赠送课时折价系数**：可为每门课程单独配置，默认值为1。用于核算已消耗赠送课时的退费成本。
...
- **业务流程**：
  1. 审批人查看退费单详情，包括学员信息、订单信息、不可编辑的《退费金额明细单》、关联的发票状态（FR-INV-004）等。
  2. 系统根据退费单信息，按**预先配置的三级审批路由**自动决定当前审批流程级别和超时规则：
     - **一级（小额通道）**：退费金额 ≤ 5000元 且折扣 ≥ 9折 → 校区主管审批，30分钟超时自动通过。
     - **二级（中级风控）**：退费金额 5001-10000元 或折扣 85折-89折 → 校区主管+财务主管二级审批，工作日4小时/非工作日8小时超时自动通过。
...
- **业务规则**：
  - 审批阈值固化：`≤5000元且≥9折` 为小额通道。
  - 超时规则固化：`一级30分钟`；`二级工作日4h/非工作日8h`。
  - 审批代理人规则：审批人5分钟未处理，提醒代理人；30分钟未处理，转至值班校长。
...
- **后置状态**：规则变更；月底报表按新规则生成。
- **异常处理**：
  - 开启旷课计费时若未上传凭证，系统应阻止保存并提示。
- **验收标准**：
  - 默认配置下，一个学员旷课5次，月底收入确认报表中该学员的消课收入只包含正常出勤的课时。
...
- **业务流程**：
  - **自动开票**：
    ...
    2. 二次确认后，若订单金额低于或等于后台配置的“自动开票审核金额阈值”（如5000元），系统自动将开票请求推送至第三方电子发票平台。
...
- **触发方式**：用户访问PC后台首页或移动端App首页，系统自动加载。
- **业务流程**：
...
- **业务规则**：
  - **分级审批阈值**：
    ...
    - **校长强制审批**：退款金额 > 10000元 或 折扣 < 85折 或 调课节数 > 150课时 或 捆绑销售。工作日≤4h、非工作日≤8h处理，绝不自动通过。
...
- **触发方式**：每日18:00系统自动触发；任何资金变动操作（退费/补款/单边账认领）即时触发增量对账。
- **业务流程**：
...
（到此，所有经营管理子系统的需求条目已被完整覆盖，并加入了相关CCB裁决的业务规则。）
...
- **业务流程**：
  ...
  6. 所有变更即时生效，并写入审计日志
- **业务规则**：
  - 组织节点类型包括：校区、部门、班组，校区节点下可挂部门或班组，部门下可挂班组
  - 总部根节点为系统初始化时创建，不可删除
  - 数据权限基于组织节点向下覆盖：父节点角色可查看其下所有子节点的数据
...
- **验收标准**：
  1. 日志记录包含操作时间、操作人、IP、设备信息、模块、动作、改前值、改后值（通过触发操作后查询日志验证）
  2. 系统内任何角色均无删除、修改审计日志的功能权限（分别测试超级管理员、运维账号无此类入口或API）
...
- **业务流程**：
  ...
  5. 业务异常告警（如审批超时、对账异常）统一汇入告警中心，按严重级别+角色过滤显示
...
- **业务流程**：
  ...
  2. 若5次重试全部失败，系统在24小时内执行主动查单对账任务：调用支付渠道订单查询接口获取实际支付状态
  3. 主动查单后，若支付成功但系统未入账，该笔记录自动进入“单边账报表”
  4. 系统每日18:00日切（根据 CCB-ISSUE-014 裁决）自动生成日终对账报告，同时生成“单边账报表”，列出银行已扣款但系统未成功入账的记录
  5. 任何资金变动操作（退费、补款、单边账认领）即时触发一次增量对账
  6. 财务人员查看“单边账报表”，对单边账记录进行审核确认，执行补录或冲正操作
  7. 主动查单、单边账冲正等人工操作均关联至操作人并进入独立审计日志
- **业务规则**：
  - 回调重试：至少5次，间隔指数递增
  - 主动查单：重试全失败后24小时内执行
  - 单边账报表：每日生成，包含：商户订单号、渠道流水号、支付金额、银行扣款时间、系统入账状态
...
- **验收标准**：
  ...
  2. 重试全失败后24小时内执行主动查单对账任务（模拟回调持续失败验证）
  3. 每日生成单边账报表，列出未成功入账记录（查看日终报告验证）
